defmodule PrepMechWeb.PoolChannelTest do
  ...
  test "pushes order_created with a summary when one is broadcast", %{socket: _socket} do
    order = insert(:order, status: :pending, line_items: [build(:line_item, name: "Channelmilk")])

    Phoenix.PubSub.broadcast(PrepMech.PubSub, "orders:pool", {:order_created, order})

    assert_push "order_created", %{summary: "Channelmilk"}
  end
